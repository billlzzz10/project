...
class ImageGenerationRequest(BaseModel):
    prompt: str = Field(
        ..., description="Prompt text for image generation", min_length=1
    )
    style: Optional[str] = Field(
        None, description="Image style, e.g., 'photorealistic', 'cinematic', 'anime'"
    )
...
class ImageGenerationResponse(BaseModel):
    ...
    from_cache: bool = Field(
        ..., description="True if the image was retrieved from cache"
    )
