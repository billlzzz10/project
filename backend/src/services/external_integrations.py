...
class GoogleDriveService:
    ...
    def create_backup(self, data, backup_name):
        ...
        filename = (
            f"backup_{backup_name}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def sync_data(self, sheet_name, data):
        ...
        if isinstance(data, list) and len(data) > 0:
            if isinstance(data[0], dict):
                ...
                rows = [headers] + [
                    [item.get(key, "") for key in headers] for item in data
                ]
            else:
                rows = [data]
        else:
...
class IntegrationManager:
    ...
    def backup_all_data(self):
        ...
        try:
            ...
            backup_data = {
                ...
                "chat_sessions": [
                    session.to_dict() for session in ChatSession.query.all()
                ],
                "chat_messages": [
                    message.to_dict() for message in ChatMessage.query.all()
                ],
                "prompt_templates": [
                    prompt.to_dict() for prompt in PromptTemplate.query.all()
                ],
                "generated_tools": [
                    tool.to_dict() for tool in GeneratedTool.query.all()
                ],
            }
    ...
    def generate_daily_report(self):
        ...
        try:
            ...
            # Get Airtable stats
            airtable_stats = self.airtable_service.get_records_count(
                "default_base", "default_table"
            )
    ...
    def test_all_connections(self):
        ...
        results = {
            "google_drive": {
                "status": "simulated",
                "message": "Connection OK (simulated)",
            },
            "google_sheets": {
                "status": "simulated",
                "message": "Connection OK (simulated)",
            ...
            "notion": {
                "status": (
                    "simulated" if self.notion_service.api_key else "not_configured"
                ),
                "message": (
                    "API key configured"
                    if self.notion_service.api_key
                    else "API key not configured"
                ),
            },
            "airtable": {
                "status": (
                    "simulated" if self.airtable_service.api_key else "not_configured"
                ),
                "message": (
                    "API key configured"
                    if self.airtable_service.api_key
                    else "API key not configured"
                ),
            },
        }
