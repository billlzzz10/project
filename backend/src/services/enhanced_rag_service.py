...
class EnhancedRAGService:
    def __init__(self):
        ...
        if missing_vars:
            raise ValueError(f"Pinecone configuration incomplete. Please set the following environment variables: {', '.join(missing_vars)}")
        ...
        if not self.hf_token:
            raise ValueError("Hugging Face Token must be configured. Please set the HF_TOKEN environment variable.")
        self.hf_client = InferenceClient(token=self.hf_token)
        self.embedding_model = current_app.config.get('EMBEDDING_MODEL', "intfloat/multilingual-e5-large")
    ...
    def _embed(self, text: str, mode: str = "passage") -> List[float]:
        ...
        response = self.hf_client.feature_extraction(model=self.embedding_model, inputs=[formatted_text])
        # The response is a nested list, we take the first element's mean
        vector = np.mean(response[0], axis=0).tolist()
        return vector

    def add_document(self, content: str, user_id: int, source_type: str, source_id: str, title: str) -> RAGDocument:
        """
    ...
    def get_rag_context(self, query: str, user_id: int, max_context_length: int = 4000) -> str:
