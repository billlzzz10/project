...
class AnalyticsService:
    ...
    def get_chat_analytics(self, user_id=None, days=30):
        ...
        try:
            ...
            results = (
                query.group_by(func.date(ChatMessage.created_at))
            ...
            data = [
                {"date": str(r.date), "message_count": r.message_count} for r in results
            ]
    ...
    def get_file_analytics(self, user_id=None):
        ...
        try:
            ...
            file_types = (
                file_type_query.group_by(UploadedFile.file_type)
                .order_by(desc("count"))
                .all()
            )
            ...
            total_size = (
                db.session.query(func.sum(UploadedFile.file_size)).scalar() or 0
            )

            # Get recent uploads
            recent_uploads = (
                UploadedFile.query.order_by(UploadedFile.created_at.desc())
                .limit(5)
                .all()
            )
    ...
    def get_rag_analytics(self, user_id=None):
        ...
        try:
            ...
            source_query = db.session.query(
                RAGDocument.source_type, func.count().label("count")
            )
            ...
            sources = (
                source_query.group_by(RAGDocument.source_type)
                .order_by(desc("count"))
                .all()
            )
            ...
            # Convert to list of dicts
            source_data = [
                {"source_type": s.source_type, "count": s.count} for s in sources
            ]

            # Get total counts
            total_documents = RAGDocument.query.count()

            return {"source_types": source_data, "total_documents": total_documents}

        except Exception as e:
            print(f"Error getting RAG analytics: {e}")
            return {"source_types": [], "total_documents": 0}
    ...
    def get_system_analytics(self):
        ...
        try:
            ...
            return {
                ...
                "graph": {"node_count": node_count, "edge_count": edge_count},
                "chat_stats": {
            ...
            return {
                ...
                "chat_stats": {"total_sessions": 0, "total_messages": 0},
    ...
    def get_notion_analytics(self, user_id=None):
        ...
        try:
            ...
            notion_docs = RAGDocument.query.filter(RAGDocument.source_type == "notion")
            ...
            recent_pages = (
                notion_docs.order_by(RAGDocument.created_at.desc()).limit(5).all()
            )
            ...
            return {"total_pages": notion_count, "recent_pages": recent_pages_data}

        except Exception as e:
            print(f"Error getting Notion analytics: {e}")
            return {"total_pages": 0, "recent_pages": []}
    ...
    def generate_mock_data(self):
        ...
        chat_activity = [
            {"date": d.strftime("%Y-%m-%d"), "message_count": np.random.randint(5, 50)}
            for d in dates
        ]
        ...
        system_stats = {
            ...
            "work_item_count": 120,
            "board_count": 15,
            "graph": {"node_count": 350, "edge_count": 420},
            "chat_stats": {"total_sessions": 85, "total_messages": 1250},
            "file_stats": {"total_files": 120, "total_size": 212000000},
        }
        ...
        return {
            ...
            "rag_analytics": {"source_types": rag_sources, "total_documents": 150},
