...
def share_content():
    ...
    try:
        ...
        title = data.get(
            "title", f'Shared content {datetime.now().strftime("%Y-%m-%d %H:%M")}'
        )
        ...
        if format == "md":
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(
                    f"<!DOCTYPE html><html><head><title>{title}</title></head><body><h1>{title}</h1>{html_content}</body></html>"
                )
...
def list_shared_content():
    ...
    try:
        ...
        items = (
            SharedContent.query.filter_by(user_id=user_id)
...
def get_embed(embed_id):
    ...
    try:
        shared_content = SharedContent.query.filter_by(
            share_id=embed_id, format="embed"
        ).first_or_404()
