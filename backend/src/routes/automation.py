...
def add_custom_job():
    ...
    result = current_app.scheduler_service.add_custom_schedule(
        job_name, schedule_time, predefined_jobs[job_name]
    )
    ...
    try:
        if frequency == "daily":
            result = current_app.scheduler_service.add_custom_schedule(
                "daily_backup",
                backup_time,
                current_app.scheduler_service.daily_backup_job,
            )
        elif frequency == "weekly":
            result = current_app.scheduler_service.add_custom_schedule(
                "weekly_backup",
                backup_time,
                current_app.scheduler_service.weekly_backup_job,
            )
        else:
    ...
    try:
        if frequency == "daily":
            result = current_app.scheduler_service.add_custom_schedule(
                "daily_report",
                report_time,
                current_app.scheduler_service.daily_report_job,
            )
        else:
            return (
                jsonify({"error": "Only daily frequency is supported for reports"}),
                400,
            )
...
def get_system_health():
    ...
    health_status = {
        ...
        "scheduler": {
            "status": (
                "running" if current_app.scheduler_service.is_running else "stopped"
            ),
            "jobs_count": len(
                current_app.scheduler_service.get_schedule_status().get("jobs", [])
            ),
        },
        "integrations": current_app.integration_manager.test_all_connections(),
        "database": {"status": "connected", "last_backup": "2024-01-15T02:00:00Z"},
