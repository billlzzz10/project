...
def get_chat_sessions():
    ...
    return jsonify([{'id': session.id, 'title': session.title, 'created_at': session.created_at} for session in sessions]), 200
...
def send_message(session_id):
    ...
    try:
        ...
        if use_rag:
            context = current_app.rag_service.get_rag_context(user_message, user_id=user_id)
