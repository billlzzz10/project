...
def get_prompts():
    ...
    query = query.filter(
        (PromptTemplate.created_by == user_id) | (PromptTemplate.is_public == True)
    )
...
def get_prompt_stats():
    ...
    public_prompts = PromptTemplate.query.filter_by(
        created_by=user_id, is_public=True
    ).count()
    total_usage = (
        db.session.query(db.func.sum(PromptTemplate.usage_count))
    ...
    categories = (
        db.session.query(PromptTemplate.category, db.func.count(PromptTemplate.id))
        .filter_by(created_by=user_id)
...
def get_tool_stats():
    ...
    tested_tools = GeneratedTool.query.filter_by(
        created_by=user_id, is_tested=True
    ).count()
    total_usage = (
        db.session.query(db.func.sum(GeneratedTool.usage_count))
        .filter_by(created_by=user_id)
        .scalar()
        or 0
    )
    ...
    languages = (
        db.session.query(GeneratedTool.language, db.func.count(GeneratedTool.id))
        .filter_by(created_by=user_id)
        .group_by(GeneratedTool.language)
        .all()
    )

    categories = (
        db.session.query(GeneratedTool.category, db.func.count(GeneratedTool.id))
        .filter_by(created_by=user_id)
        .group_by(GeneratedTool.category)
        .all()
    )
